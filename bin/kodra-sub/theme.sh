...
get_nvim_colorscheme() {
    ...
    case "$theme" in
        ...
        ghostty-blue) echo "tokyonight-storm" ;;
        *)            echo "default" ;;
    esac
}
...
sync_nvim_colorscheme() {
    ...
    local colorscheme=$(get_nvim_colorscheme "$theme")
    
    if [ ! -d "$nvim_config" ]; then
        return 0  # nvim not configured
    fi
    
    # Create or update the theme sync file
    mkdir -p "$nvim_config"
    cat > "$nvim_config/kodra-theme.lua" << EOF
-- Kodra Theme Sync
-- Auto-generated by kodra theme command
-- Current theme: $theme
...
-- Apply colorscheme if plugin is available
function M.apply()
    local ok, _ = pcall(function()
        vim.cmd("colorscheme " .. M.colorscheme)
    end)
    if not ok then
        -- Fallback if colorscheme not installed
...
apply_theme() {
    ...
    # Neovim colorscheme sync (#52)
    sync_nvim_colorscheme "$theme"
    ...
    # Hot-reload components (#48)
    if [ "${KODRA_NO_REFRESH:-}" != "1" ]; then
        bash "$KODRA_DIR/bin/kodra-sub/refresh.sh" all 2>/dev/null || true
    fi
}
